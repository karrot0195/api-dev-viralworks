...
export class MailPostContentState implements IMailState {
    ...
    getContent(kolName: string, jobLink: string, action: string) {
        const templatePath = process.cwd() + `/Resources/Views/Mail/Post/content-${action}.pug`;
    ...
    getSubject(jobTitle: string) {
        return `[ViralWorks] Thông báo về nội dung bài đăng: [${jobTitle}]`;
    }
