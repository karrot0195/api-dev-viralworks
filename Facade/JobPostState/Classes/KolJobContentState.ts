...
export class KolJobContentState implements IKolJobState {
    async accept(context: KolJobStateContext, session: ClientSession) {
        ...
        if (mailResponse) {
            ...
            subject.post.status = KolJobPostStatus.Content;

            const data = {
                causer_id: context.getCauserId(),
                job_post_status: KolJobPostStatus.Content,
    ...
    public getMailType() {
        return MailType.POST_JOB_CONTENT;
    }
}
