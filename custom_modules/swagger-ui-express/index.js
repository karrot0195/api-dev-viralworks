...
var generateHTML = function (swaggerDoc, opts, options, customCss, customfavIcon, swaggerUrl, customeSiteTitle) {
  ...
  if (typeof customJs == 'string' && customJs !== '') {
    customJsHTML = `<script src="${customJs}"></script>`;
  } else if (Array.isArray(customJs)) {
    for (let i = 0; i < customJs.length; i++) {
      const jsSrc = customJs[i];

      if (jsSrc !== '') {
        customJsHTML += `<script src="${jsSrc}"></script>`;
      }
    }
  }

  var htmlWithCustomJs = htmlWithFavIcon.replace('<% customJs %>', customJsHTML);
